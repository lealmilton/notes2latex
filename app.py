...
# Create the progress bar only once after the file is uploaded
if uploaded_file is not None and not st.session_state['progress_bar_active']:
    st.session_state['progress_bar'] = st.progress(0)
    st.session_state['progress_bar_active'] = True

if not st.session_state['processing_completed'] and uploaded_file is not None:
    with tempfile.NamedTemporaryFile(delete=False, suffix='.pdf') as tmp:
        tmp.write(uploaded_file.getvalue())
        pdf_path = tmp.name

    if pdf_path:
        with st.spinner("Well, it's AI. It takes time. Gpus are running somewhere. Go read a theorem and come back soon 😂."):            # Convert PDF to PNG
